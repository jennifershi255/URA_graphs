...
# ── NEW: Query HF API to find models trained on a specific dataset ─────
def find_models_for_dataset(api, dataset_name, pipeline_tag="text-classification", max_models=500):
    ...
    """
    try:
        # Use trained_dataset parameter to filter by dataset
        # Note: sort parameter sorts by the field, no separate direction needed
        ...
        # Manually filter by pipeline_tag since not all models have it set correctly
        # We'll collect all and filter during processing
        return list(models)
...
def extract_model_info_from_api(api, model_id):
    ...
    Returns parsed dict similar to parse_model_card.
    """
    try:
...
# ── NEW: Dataset-centric collection approach ───────────────────────────
def collect_by_dataset_approach(target_records=7500):
    """
    NEW APPROACH: Start with popular datasets, then find ALL models trained on each.
    This creates much denser records showing dataset->models relationships.
    """
    # Use the global authenticated api
    global api
    ...
    for i, row in enumerate(filtered.take(20000)):  # Sample more for better dataset discovery
        if i % 2000 == 0:
    ...
    for idx, dataset_name in enumerate(top_datasets):
        ...
        for model in models:
            ...
            # Extract detailed info from model card
            parsed = extract_model_info_from_api(api, model_id)
            if not parsed:
                continue
            
            # We know this model uses this dataset (from the query)
            # Add a row for this (model, dataset) pair
            flat_rows.append({
                "model": parsed["model"],
                "base_model": parsed["base_model"],
                "finetuned_dataset": dataset_name,  # The dataset we queried for
...
if __name__ == "__main__":
    # Run the new dataset-centric approach
    ...
    if flat_rows:
        ...
        print("Saved to records_dense.csv")
    else:
